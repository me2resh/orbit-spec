...
const plan = { id: 'plan-1', revision: 2 };
const reconciliation = { id: 'reconciliation-1', projectSnapshotId: 'snapshot-1' };
const slice = {
  id: 'slice-1',
  outcomeId: 'outcome-1',
  objective: 'Add the audit history view',
  why: 'The reconciliation found no operator review surface.',
  basedOn: { reconciliationId: 'reconciliation-1', repositories: { demo: 'abc123' } },
...
test('dry-run never invokes GitHub and returns the proposed payload', async () => {
  let invoked = false;
  const result = await syncGitHub({ plan, reconciliation, slice, repo: 'me2resh/demo', dryRun: true, execCommand: async () => { invoked = true; } });
...
test('fails clearly when GitHub authentication is unavailable', async () => {
  await assert.rejects(
    syncGitHub({ plan, reconciliation, slice, repo: 'me2resh/demo', execCommand: async () => { throw new Error('not authenticated'); } }),
...
test('creates one issue and adds it to Projects v2 when configured', async () => {
  ...
  const result = await syncGitHub({
    plan, reconciliation, slice, repo: 'me2resh/demo', projectOwner: 'me2resh', projectNumber: '7',
